...
# Follow-up: direct server-side mount teaching

# Quest companion: quest-source item at game objects

Implement reusable data-driven support for objectives such as Crown of the Earth, where a companion must use a vial/phial at a specific pool/gameobject. Inspect exact quest/objective/source-item/gameobject relations in this 3.3.5a database/DBC; do not hard-code only one quest. Detect the required source item in bags, resolve its target gameobject, find nearby matching objects using live map/position and interaction range, verify the objective is incomplete and the item usable, then safely target/interact/use it and confirm progress. Avoid combat interruption, hostile objects, movement loops, repeated spam and unrelated item consumption; preserve corpse/herb/mining/skinning behaviour. Add focused tests and a manual Crown of the Earth verification plan. Build but do not install/restart until Codex review and owner approval; report files, commands, backups and rollback steps.

The live test proved the 3.3.5a client blocks opposing-race mount items locally and sends no use packet, so the AllowableRace exemption alone cannot work. Implement the robust route: extend `mod-web-admin` with a narrowly validated command/API operation that, for an online named player and a mount item, resolves the item’s taught mount spell from the item template and calls AzerothCore's native player spell-learning path directly. Preserve the normal item grant, validate that the item is a real mount, reject missing/invalid/non-mount teach spells, and do not bypass class/level/riding/training requirements unless native spell learning itself requires them. Keep audit output clear (item, spell, player, result). Update the Mounts API/UI to invoke this operation when giving a mount (or expose an explicit “teach mount” action) without direct DB writes. Add tests and build the actual Linux worldserver; back up source and installed binary, then report exact rollback steps. Do not deploy/restart until Codex reviews the implementation and owner approves.
