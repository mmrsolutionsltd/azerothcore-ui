...
It also adds **Caster Auto-Attack** for real spellcaster characters. A short
right-click on a hostile creature in the 3D world toggles repeated casting of the
character's selected learned filler spell. Camera drags are ignored. A small spell
button beside the target frame shows whether the feature is ready or running;
left-click it to toggle and right-click it to cycle available filler spells. Movement
and manually cast spells pause the filler temporarily, while changing target, dying,
or losing the target stops it.
...
The module and addon currently use companion protocol version 10. The panel reports a
...
- `/cauto` shows the selected caster-auto spell and commands.
- `/cauto next` cycles the learned filler spells.
- `/cauto <spell name>` selects a specific learned filler spell.
- `/cauto on`, `/cauto off`, and `/cauto stop` control the feature.
...
Caster Auto-Attack requires the matching `mod-web-admin` source to be rebuilt,
installed and the worldserver restarted. The server validates that the spell is
learned, hostile, ranged and being cast at the player's selected living enemy.
